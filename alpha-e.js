...
function master(on){s.power=on;$("powerOff").classList.toggle("active",!on);$("powerOn").classList.toggle("active",on);if(!on){s.rough=s.turbo=s.mfc=s.cooler=s.hv=s.mw=s.beam=false;s.vent=false}update();send("MasterPower","system",on?"on":"off")}
$("powerOn").onclick=()=>master(true);$("powerOff").onclick=()=>master(false);
...
function send(type,equipmentId,action=""){const f=$("alphaUnity");if(!UNITY_WEBGL_URL||!f.contentWindow)return;f.contentWindow.postMessage({source:"alpha-e-parent",type,equipmentId,action,state:{...s}},"*")}
